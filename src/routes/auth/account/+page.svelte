...
<h2 class="text-2xl">Login as: <p class="font-semibold text-pink-400">{data.session.user.email}</p></h2>
<div class="p-4">
	<form
	...
	<h2 class="text-xl font-semibold">Sponsorised profile:</h2>
	  {#if profile !== null}
		{#each profile as p}
		  <div class="mb-4">
			<label class="block mb-2 text-gray-700">Username:
			  <input type="text" name="username" bind:value={p.name} class="px-3 py-2 w-full rounded-md border focus:outline-none focus:ring focus:border-blue-300" />
			</label>
	  ...
	  <div class="text-center">
		<button
		  type="submit"
		  class="p-1 px-4 text-white bg-pink-400 rounded-md"
		  disabled={loading}
	...
	<div class="mt-4">
	  ...
	  <form action="/auth/logout" method="POST">
		<button class="rounded-md border-[1px] border-pink-400 px-2 p-1" type="submit">Logout</button>
	  </form>
	</div>
  </div>
</div>